...
export function createUI() {
  ...
  function bindControls(handlers) {
    ...
    elements.volumeSlider.addEventListener("input", () =>
      handlers.onVolumeChange(Number(elements.volumeSlider.value) / 100),
    );
    elements.contrastToggle.addEventListener("change", () =>
      handlers.onToggleContrast(elements.contrastToggle.checked),
    );
    elements.largeTextToggle.addEventListener("change", () =>
      handlers.onToggleLargeText(elements.largeTextToggle.checked),
    );
   ...
   * state: any,
   * derived: any,
   * symbolMap: Map<string, {id: string, name: string, glyph: string}>,
  ...
  function renderState(model) {
    ...
    elements.spinButton.textContent = state.isSpinning ? "Spinning..." : "Run Inference Spin";
  ...
  /**
   * @param {number} reelIndex
   * @param {{name: string, glyph: string}} symbol
   * @param {boolean} isFinal
   */
  function updateReel(reelIndex, symbol, isFinal) {
    const reel = elements.reels[reelIndex];
    const glyph = reel.querySelector(".reel-glyph");
   ...
   * @param {Map<string, {name: string, glyph: string}>} symbolMap
   */
...
/**
 * @returns {Record<string, HTMLElement | HTMLInputElement | HTMLButtonElement>}
 */
